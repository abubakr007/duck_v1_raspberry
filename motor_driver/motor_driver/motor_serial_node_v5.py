...
class MotorSerialNode(Node):
    ...
    def cmd_vel_callback(self, msg):
        ...
        left_rpm = int((left_speed / (2 * math.pi * self.wheel_radius)) * 60.0)
        right_rpm = int((right_speed / (2 * math.pi * self.wheel_radius)) * 60.0)

        msg_out = Int32MultiArray()
        msg_out.data = [left_rpm, right_rpm]
    ...
    def read_serial(self):
        if self.serial.in_waiting:
            try:
                line = self.serial.readline().decode().strip()  # ✅ read line here
                data = line.split(",")
                if len(data) == 2:
                    ...
                    # self.v_l = (self.rpm_l / 20.0) * 2 * math.pi * self.wheel_radius
                    # self.v_r = (self.rpm_r / 20.0) * 2 * math.pi * self.wheel_radius
