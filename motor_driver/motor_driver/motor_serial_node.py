...
class MotorSerialNode(Node):
    def __init__(self):
        ...
        # 🔧 Rates
        self.create_timer(0.01, self.read_serial)      # 100 Hz (was 20 Hz)
        self.create_timer(0.01, self.update_odometry)  # 100 Hz (was 50 Hz)

        # Robot params
        self.wheel_radius = 0.033605  # meters (actual measured)
        self.wheel_base   = 0.1735    # meters (actual measured)

        # If firmware sends RPMs, set False; if it already sends m/s, set True
        self.IS_FIRMWARE_SPEED_MPS = True
    ...
    def cmd_vel_callback(self, msg):
        ...
        # Send PWM values to serial writer
    ...
    def read_serial(self):
        if self.serial.in_waiting:
            try:
                ...
                data = line.split(",")
                if len(data) == 2:
                    if self.IS_FIRMWARE_SPEED_MPS:
                        self.v_l = float(data[0])
                        self.v_r = float(data[1])
                        self.rpm_l = self.v_l   # keep publishing on motor_rpms for compatibility
                        self.rpm_r = self.v_r
                    else:
                        # Firmware sends RPM → convert to m/s
                        self.rpm_l = float(data[0])
                        self.rpm_r = float(data[1])
                        self.v_l = self.rpm_l * 2.0 * math.pi * self.wheel_radius / 60.0
                        self.v_r = self.rpm_r * 2.0 * math.pi * self.wheel_radius / 60.0
    ...
    def update_odometry(self):
        ...
        # ✅ Add reasonable covariances (2D diff-drive assumptions)
        # Pose covariance (x, y ~0.05 m^2, yaw ~0.1 rad^2; z/roll/pitch huge)
        ...
        # Twist covariance (vx, wz modest; others huge)
        ...
        delta_rad_l = (v_l / self.wheel_radius) * dt
        delta_rad_r = (v_r / self.wheel_radius) * dt
