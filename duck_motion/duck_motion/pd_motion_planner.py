...
class PDMotionPlanner(Node):
    ...
    def control_loop(self):
        ...
        twist_stamped_msg.twist = cmd_vel
        self.cmd_pub_out.publish(twist_stamped_msg)
    ...
    def transform_plan(self, frame):
        ...
        # Transform from plan frame (source) to target `frame`
        transform_matrix = quaternion_matrix([
            transform.transform.rotation.x,
            transform.transform.rotation.y,
            transform.transform.rotation.z,
        ...
        for pose in self.global_plan.poses:
            ...
            # ✅ CORRECT ORDER: T_target_source * T_pose_source
            transformed_pose = concatenate_matrices(transform_matrix, pose_matrix)

            tx, ty, tz = translation_from_matrix(transformed_pose)
            qx, qy, qz, qw = quaternion_from_matrix(transformed_pose)
            ...
            pose.header.frame_id = frame  # optional, but consistent
